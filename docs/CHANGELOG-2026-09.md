...
## 12. v3.1：路由与一致性修复（2026-09-24）

这一轮修的问题大多来自同源项目 EchoMind 问答区里别人指出过的缺陷，逐条核对后发现 GoEuroOps 也有。

| 问题 | 修法 | 位置 |
|---|---|---|
| 关键词"细化"会跨领域改写意图，比如把"留学咨询"改成"退款" | 细化只在同一意图大类内进行 | `core/intent_recognizer.py` `_vote` |
| 大类和细类分数不合并：费用 0.385 + 退款 0.12 本该过线，却因为分开比较被判成 OTHER | 两层投票：先按意图大类汇总选领域，再在领域内选细分意图 | 同上 |
| LLM 判错且自报置信度 ≥ 0.72 时，向量和关键词两路改不回来（它们合计最多 0.3） | 分歧检测：两路都强烈指向另一领域（向量 ≥ 0.6、关键词 ≥ 0.5），并且消息里没有 LLM 所选领域的任何关键词时，降为低置信 OTHER，交给澄清追问；有两个领域的关键词时视为复合问题，不触发 | `_deterministic_conflict` |
| 运行中上传知识库文档后，检索缓存最多 300 秒内返回旧结果 | 写入后立即清空检索缓存（`MCPToolManager.invalidate_cache`） | `api/main.py`、`mcp/tool_manager.py` |
| 同一会话多次要求真人会重复建交接单 | 交接单按会话幂等：已有未关闭的交接单时，把新消息追加到它的 followups，回复告知"不会重复排队"；顾问关单后再要求真人才开新单 | `business/lead_store.py` `create_handoff` |
| "我想申请退款，选校报告还没交付"会把咨询 Agent 也拉进来（"申请""选校"命中咨询协作词），耗时 11.9s | 复合问题检测前先去掉业务目录里的服务名、交付物名和"申请退款"这类费用短语；修复后只走费用 Agent，耗时 7.1s | `agents/agent_orchestrator.py` `_collaboration_targets` |

测试从 95 个增加到 108 个：新增 `test_intent_vote.py`（投票与分歧检测 8 个）、`test_tool_cache.py`（缓存失效 2 个），以及交接单去重和交付物误触发的回归测试。已在 Docker 环境用真实请求验证路由、复合问题和交接单去重。

## 13. v3.2：意图评测基准与向量路升级（2026-09-24）

### 13.1 自建意图评测集

- `backend/evaluation/datasets/intent_eval_v1.jsonl`：19 类 × 20 条 = 380 条，类别均衡；每条带难点标签（口语、错别字、中英混杂、依赖上文、边界混淆）。样本定义在 `build_intent_v1.py`，标注规则和有争议样本见 `LABELING.md`。
- `backend/evaluation/intent_benchmark.py`：每条样本只调一次 LLM，同时算出五种识别器的结果（线上融合、修复前融合、仅 LLM、仅向量、仅关键词）和"LLM 故障时的降级"结果；跑之前检查样本是否和内置模板重复，防止分数虚高。报告写到 `evaluation/reports/`。

### 13.2 结果（最终配置，3 次运行平均）

| 识别器 | 准确率 | Macro-F1 | 领域准确率 |
|---|---|---|---|
| 三路融合（线上） | 91.7% | 0.915 | 93.9% |
| 仅 LLM | 91.7% | 0.915 | 93.9% |
| 仅向量（bge） | 59.2% | 0.565 | 66.0% |
| 仅关键词 | 38.7% | 0.449 | 41.3% |
| LLM 故障时的降级 | 59.2% | 0.565 | 66.0% |

发现：
1. **融合在正常情况下不提高准确率**，和仅 LLM 持平；向量和关键词两路的价值在 LLM 故障时的降级。
2. 最常见的错误是 escalation 被判成 human_handoff，两者路由到同一个 Agent，所以领域准确率比意图准确率高约 2 个百分点。真正跨 Agent 的主要错误是"问工作室本身"被判成服务询价。
3. 边界混淆样本准确率约 73%，依赖上文和中英混杂的样本全对。

### 13.3 据此做的改动

| 改动 | 依据 |
|---|---|
| 意图识别的向量路从字符 n-gram 换成和知识库共享的 bge-small-zh（`get_shared_embedding_function`，只加载一份模型），可用 `GOEUROOPS_INTENT_EMBEDDING=ngram` 关闭 | 仅向量准确率 40.0% → 59.2%，LLM 故障时的降级准确率随之提升；每条约 1ms |
| 分歧检测的向量阈值按向量类型区分：bge 0.8、n-gram 0.6 | 换 bge 后沿用 0.6，误触发 3 次且全是把正确结果改错；bge 相似度 ≥ 0.8 时模板匹配精确率 95%，高于 LLM |
| 分歧检测只在两路指向具体业务领域时生效，问候、请求、查询、反馈、投诉、其他不能否决 LLM | 校准后唯一的误触发是"帮我改成用邮件联系我"被两路判成 request，否决了正确的 account |

测试增加到 109 个。

## 14. 仓库公开前的文档清理（2026-09-24）

删除 `backend/wiki/` 下旧版通用客服时期的文档（学习文档、定位与技术亮点、技术亮点、架构图、完整使用指南、业务流程说明、重点代码、文档中心）及其配图。这些文档描述的是改造前的 Technical/Billing/General 电商客服架构，与现在的系统不一致。当前的设计说明以根目录 README、`backend/README.md`、`backend/skills/README.md` 和 `backend/wiki/Skills与意图门控RAG.md` 为准。

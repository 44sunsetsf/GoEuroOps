  ...
  <main :class="['app-shell', `app-shell-${activeView}`]">
          ...
          <div class="messages" ref="messageList">
            ...
            <div v-if="messages.length === 0" class="empty-state">
              ...
              <div class="starter-prompts">
                ...
                <button @click="usePrompt('瑞典的英语授课计算机硕士项目大概是什么样的？')">留学咨询</button>
    ...
    <section v-else-if="activeView === 'knowledge'" class="page page-knowledge">
      <div class="page-heading">
        <div class="heading-copy">
          <h1>知识库</h1>
          <p>维护运营助手回答留学咨询问题时引用的参考资料——五国项目概况、服务介绍和申请材料说明。</p>
        </div>
    ...
    <section v-else class="page page-evaluation">
      <div class="page-heading">
        <div class="heading-copy">
          <h1>评测助手</h1>
          <p>运行内置评测，查看意图识别准确率、回复质量和回归结果。</p>
        </div>
  ...
  try {
    await streamChat(settings, content, {
      ...
      async onDone(response) {
        ...
        assistantMessage.meta = [response.intent, response.primaryAgent || response.agentType, response.knowledgeUsed ? 'RAG' : '', response.escalated ? '转人工' : ''].filter(Boolean).join(' · ')
        await loadMonitor()
      },
      onError(error) {
        assistantMessage.streaming = false
